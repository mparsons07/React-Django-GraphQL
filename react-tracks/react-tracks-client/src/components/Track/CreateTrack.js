...
const CreateTrack = ({ classes }) => {
  ...
  const handleAudioChange = event => {
    ...
    if(selectedFile && selectedFile.size > fileSizeLimit){
      setFileError(`${selectedFile.name}: selected file is too large.`)
    }else {
  ...
  const handleAudioUpload = async () => {
    try {
      ...
      data.append('cloud_name', 'mattborghi')
      const res = await axios.post('http://api.cloudinary.com/v1_1/mattborghi/raw/upload', data)
      return res.data.url
    } catch(err) {
  ...
  const handleSubmit = async (event, createTrack) => {
    event.preventDefault() // prevents form for reloading page
    setSubmitting(true)
  ...
  const handleUpdateCache = (cache, { data: { createTrack } } ) => {
    ...
    cache.writeQuery({ 
      query: GET_TRACKS_QUERY, data: { tracks }
     })
  }
  ...
  return(
    ...
    <Mutation 
      ...
      onCompleted={data => {
        ...
        setSubmitting(false)
        setOpen(false)
        // Clear form when song is created
        setTitle("")
        setDescription("")
        setFile("")
      }}
      // refetchQueries: queries to be done after completed mutation
      // refetchQueries={data => [{
      //   query: GET_TRACKS_QUERY
      // }]}
